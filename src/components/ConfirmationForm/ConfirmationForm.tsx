...
const ConfirmationForm: React.FC = () => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={styles.form}>
      {fields.map((field, index) => {
        ...
        return (
          <fieldset key={field.id} className={styles.guestSection}>
            ...
            <legend className={styles.guestLegend}>Invitado {index + 1}</legend>
             {fields.length > 1 && ( <button type="button" onClick={() => remove(index)} className={styles.removeButton}>×</button> )}
             ...
             <div className={styles.formField}>{/* Tipo */}
                <label htmlFor={`guests.${index}.tipo`}>Eres... *</label>
            ...
            {guestType === 'adulto' && (
              <>
                ...
                <div className={styles.formField}>
                     ...
                     {/* Puedes añadir validación si este campo fuera obligatorio */}
                </div>
      ...
      <button type="button" onClick={() => append(defaultGuest)} className={styles.addButton}>+ Añadir otro invitado</button>
      {submitError && <p className={styles.submitErrorMsg}>{submitError}</p>}
      <button type="submit" className={styles.submitButton} disabled={isSubmitting || !isValid}>{isSubmitting ? 'Enviando...' : 'Confirmar Asistencia'}</button>
    </form>
  );
};
